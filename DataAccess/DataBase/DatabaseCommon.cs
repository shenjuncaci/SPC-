...
namespace DataAccess
{
    public class DatabaseCommon
    {
        ...
        public static object GetATField<T>()
        {
            Type objTye = typeof(T);
            string _KeyField = "";
            AutoIncreaseAttribute KeyField;
            var name = objTye.Name;
            foreach (Attribute attr in objTye.GetCustomAttributes(true))
            {
                KeyField = attr as AutoIncreaseAttribute;
                if (KeyField != null)
                    _KeyField = KeyField.Name;
            }
            return _KeyField;
        ...
        {
            Assembly asmb = Assembly.LoadFrom(System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "bin", "Entity.dll"));
        ...
        {
            //string atName = GetATField<T>().ToString();
            Type type = entity.GetType();
            StringBuilder sb = new StringBuilder();
            sb.Append(" Insert Into ");
            sb.Append(type.Name);
            ...
            {
                if (prop.GetValue(entity, null) != null && GetATField<T>().ToString() != prop.Name)
                {
