        ...
        renderRowData: function ($self, data) {
            ...
            for (var i = 0; i < len; i++) {//行循环
                ...
                if (dfop.isMultiselect) {
                    ...
                    var $dcell = $('<div class="jfgrid-data-cell jfgrid-tool-cell" rownum="' + rownum + '" colname="jfgrid_multiselect" ><input role="checkbox" type="checkbox" ' + ck + ' ></div>');
                    $letf.append($dcell);
                    $lastCell = $dcell;
                }
                if (dfop.isSubGrid) {
            ...
            function _fn(_data, deep, path) {
                for (var j = 0, l = _data.length; j < l; j++) {
                    ...
                    if (dfop.isMultiselect) {
                        ...
                        var $dcell = $('<div class="jfgrid-data-cell jfgrid-tool-cell" rownum="' + rownumindex + '" colname="jfgrid_multiselect" ><input role="checkbox" type="checkbox" ' + ck + ' ></div>');
                        $letf.append($dcell);
                        $lastCell = $dcell;
                    }
                    if (dfop.isSubGrid) {
            ...
            if (!!node.editType) {
                switch (node.editType) {
                    ...
                    case 'checkbox':
                        ...
                        $dcell.html($input);
                        break;
                    case 'datetime':       // 添加明细表日期控件，目前还有点bug，无法监听到WdatePicker修改后的数据,20190115已修复bug，目前完美，配合bootstrap-datetimepicker使用
                        var $input = $('<input  type="text" class="form-control"  />').val(value || '');
                        $input.css({ 'text-align': node.align });
                        ...
                        //使用了日期控件以后无法监听到值变化，找到bootstrap的日期控件可以有监听日期改变的接口，愉快~~
                        //$input.on('input propertychange', function () {
