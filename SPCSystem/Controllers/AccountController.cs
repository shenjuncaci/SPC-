...
namespace SPCSystem.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult ChangePasswordAjax(string OldPassword,string NewPassword)
        {
            ...
            {
                return Content(new JsonMessage { Success = false, Code = "-1", Message = "输入的老密码不正确，请重新输入" }.ToString());
            }
            else
            {
                Base_UserBll base_userbll = new Base_UserBll();
                ...
                {
                    return Content(new JsonMessage { Success = true, Code = "1", Message = "修改成功，请使用新密码登录" }.ToString());
                }
                else
                {
                    return Content(new JsonMessage { Success = false, Code = "-1", Message = "修改失败，没有找到匹配的记录" }.ToString());
                }
            }
        }
    }
